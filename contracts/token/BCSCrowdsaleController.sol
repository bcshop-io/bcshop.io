...
contract BCSCrowdsaleController is Owned {
    ...
    function initBeneficiaries(address _beneficiary, address _devTeamTokenStorage, address _miscTokenStorage) ownerOnly {
    ...
    function createPreIco(uint256 startTime, uint256 durationInHours) ownerOnly {
        require(address(preIcoSale) == 0x0);

        preIcoSale = new BCSCrowdsale(token, beneficiaryWallet, startTime, durationInHours, 0, PREICO_TOKENS_FOR_ONE_ETHER);
    ...
    function finalizePreico() ownerOnly {
        ...
        require(preIcoSale.balance > 0);

        preIcoSale.transferToBeneficiary();                
    }

    /**@dev Step 4. Create ico crowdsale */
    function createIco(uint256 startTime, uint256 durationInHours) ownerOnly {
        require(address(icoSale) == 0x0);                

        icoSale = new BCSCrowdsale(token, beneficiaryWallet, startTime, durationInHours, MINIMUM_SALE_GOAL, ICO_TOKENS_FOR_ONE_ETHER);
    ...
    function finalizeIco() ownerOnly {        
        ...
        require(icoSale.balance > 0);

        icoSale.transferToBeneficiary(); 

        beneficiaryWallet.lock(FUNDS_COMPLETE_UNLOCK_DAYS);

        token.approve(icoSale, 0);
    ...
    * temp dev methods 
    ***********************************************************************************************/
    function setPreico(address newPreIco) {
        preIcoSale = BCSCrowdsale(newPreIco);
        preIcoSale.transferOwnership(this);
        token.approve(preIcoSale, TOKEN_CAP * TOKEN_PREICO_SALE_PCT / 100);
    }
    function setIco(address newIco) {
        icoSale = BCSCrowdsale(newIco);
        icoSale.transferOwnership(this);
        token.approve(icoSale, token.balanceOf(this));
    }
