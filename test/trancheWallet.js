...
contract("TokenTrancheWallet", function(accounts) {    
    ...
    it("check withdrawable amount before lock, should equal all the balance", async function() {
        result = await wallet.amountAvailableToWithdraw.call();
        assert.equal(result[1], 0, "Invalid tranche: should be 0");
        assert.equal(result[0], WalletTokens, "Invalid withdrawable amount");
    })
    ...
    it("try to lock again for lesser timeout, should fail", async function() {
        try {
            await wallet.lock(1);
        } catch(e) {
            return true;
