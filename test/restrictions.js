...
contract('ParticipantInvestRestrictions: 1 total allowed, 0 reserved', function(accounts) {
    it('initial allowance', async function() {
        ...
        assert.equal(await restrictions.canInvest.call(investor1, floorEther / 2), false, "investor1 shouldn't be able to invest");
        assert.equal(await restrictions.canInvest.call(investor2, floorEther), true, "investor2 should be able to invest");
        
    })

    it('allowed investment', async function () {
    ...
    it('unallowed investment', async function() {        
        try {
...
contract('ParticipantInvestRestrictions: 2 total allowed, 2 reserved', function(accounts){
    ...
    it('reserve investor1', async function() {
        ...
        assert.equal(await restrictions.canInvest.call(investor1, floorEther / 10), false, "Invstor1 can't invest low amount however");

        assert.equal(await restrictions.canInvest.call(investor2, floorEther), false, "investor2 can't invest now");
    })        

    it('unreserve investor1', async function() {
        await restrictions.unreserveFor(investor1);
        assert.equal(await restrictions.canInvest.call(investor1, floorEther), false, "unreserved and can't invest"); 
    })
