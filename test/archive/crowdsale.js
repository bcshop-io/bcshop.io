...
contract("BCSCrowdsale. Investor1 invests half of cap, Investor2 tries to invest more than half", function(accounts) {
    ...
    it("investor2 should have 2E as overpay", async function() {
        ...
        var tx = await sale.withdrawOverpay({from:investor2});
    ...
    it("withdraw", async function() {
        await utils.timeTravelAndMine(3601);
        console.log(await sale.getState.call());
        console.log(await sale.withdrew.call());
        let expectedEth = await sale.weiCollected.call();
        let beneficiary = await sale.beneficiary.call();
        let oldBalance = await web3.eth.getBalance(beneficiary);

        let tx = await sale.transferToBeneficiary();
        console.log("Gas used: ", tx.receipt.gasUsed);
        let newBalance = await web3.eth.getBalance(beneficiary);

        assert.equal(newBalance.toNumber(), oldBalance.plus(expectedEth).toNumber(), "!!");
    })
})
