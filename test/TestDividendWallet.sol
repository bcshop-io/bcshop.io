...
import '../contracts/token/DividendWalletFixed.sol';
import '../contracts/token/BCSToken.sol';
import './AddressStorageUser.sol';

//required migrations
//3_helper_deploy.js
contract TestDividendWallet is AddressStorageUser {
    ...
    function beforeEach() {
        ...
        Assert.equal(token.getValuableTokenAmount(), TOKEN_CAP, "Invalid valuable amount");
    ...
    *   Address1 withdraws its share. Addresses 2 and 3 do the same. Wallet is empty now. */
    function test1() {
        ...
        Assert.equal(total, token.totalSupply(), "1");
        Assert.equal(wallet.deposits(), tranche1, "2");        
    ...
    *   Wallet gets more ether. Then the share should account for both tranches */
    ...
    //     Assert.equal(wallet.etherBalanceOf(this), cthis * 3 / 4, "1");        
    ...
    //     Assert.equal(wallet.etherBalanceOf(this), 0, "4");
    // }
}
